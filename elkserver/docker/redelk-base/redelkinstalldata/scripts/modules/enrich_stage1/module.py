...
Copies the host, implant, user and process context of the initial Outflank Stage1 implant line onto
every other rtops line of that implant, so that a "task" or "output" line can be filtered on the
host or the user it belongs to.

Fixed in v3, all of them in copy_data_fields():
  * It did dst["_source"][field] = src["_source"][field] for a fixed list of four fields. Stage1
    does not report all four for every implant, so the first missing one raised KeyError, which
    aborted the module for every remaining implant of that run.
  * It overwrote the destination fields wholesale, discarding whatever logstash or a later
    enrichment had already written there. The initial implant line is the *least* specific source
  ...
  * It wrote the whole cached _source back with es.update(body={"doc": ...}), reverting concurrent
    updates, and self.logger.error(..., traceback) logged the traceback *module* - literally
    "<module 'traceback' from ...>" - instead of the formatted traceback.
...
# What the initial implant line knows and the other lines do not. Whether a document actually has
...
def merge_value(initial: Any, existing: Any) -> Any:
    """Merge a value from the initial implant line into what the target line already has.
...
def build_partial(
    ...
    """Build the partial update that adds the initial implant's context to one rtops line.
    ...
    for field in fields:
        if field not in initial_source:
            # Not every implant line carries every field; copy what is there.
            continue
...
class Module:
    """Enrich Stage1 rtops lines with the data of their initial implant line."""
    ...
    def enrich_beacon_data(self):
        ...
        for implant_id, docs in implants.items():
            ...
            if not initial_doc:
                ...
                self.logger.debug("no initial implant line for implant %s (yet)", implant_id)
                continue
    ...
    def get_initial_beacon_doc(self, implant_id):
        """The initial implant document of an implant, or False when there is none.
