...
@pytest.mark.parametrize(
    "configured,expected",
    [({"backend_filter": "beacon-*"}, "beacon-*"), ({"backend_filter": ""}, "c2*"), ({}, "c2*")],
)
def test_httptraffic_reads_the_backend_filter(daemon_env, configured, expected):
    """Which backends count as C2 used to be the literal "c2*".

    A deployment whose haproxy backends are named anything else never alarmed on traffic that
    reached the implant - the one thing this module exists to catch.
    """
    env = daemon_env({"alarms": {"alarm_httptraffic": dict(configured, enabled=True)}})
    module = load(env, "alarm_httptraffic")

    assert module.Module().backend_filter == expected
