...
| `elastic.image_tag` | string | `""` | Tag for those images. Empty means the contents of `./VERSION`, without its leading `v` - the published image tags are plain semver (`3.0.0`), while `VERSION` and the git tag carry the `v`. |
