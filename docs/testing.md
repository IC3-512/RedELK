...
| `test_ingest_redirector.py` | Recorded HAProxy traffic travels through a real Filebeat over the mutually authenticated beats input; the documents are parsed (`source.ip`, `redir.backend.name`, `http.request.method`, `http.headers.useragent`, `source.geo.country_name`, `user_agent.name`), the production millisecond-plus-offset timestamp has no `_dateparsefailure`, and `host.name` is a single value and not an array; a rogue user agent on a `c2*` backend is tagged by `alarm_useragent` with `alarm.last_alarmed` set. |
